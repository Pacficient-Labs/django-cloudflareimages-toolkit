...
def _rename_sqlite_index(connection, table, old, new, columns):
    """Drop index ``old`` and recreate it as ``new`` on the same ``columns``.

    SQLite has no ``ALTER INDEX ... RENAME``, so a rename is a drop+create.
    Used to simulate a database whose physical index names were auto-generated
    by a different Django version than the one whose names 0006 hard-codes.
    """
    cols = ", ".join(f'"{c}"' for c in columns)
    with connection.cursor() as cursor:
        cursor.execute(f'DROP INDEX "{old}"')
        cursor.execute(f'CREATE INDEX "{new}" ON "{table}" ({cols})')


# Every affected index, as (table, db-columns, legacy fixture name, a *drifted*
# name simulating a different Django version's output). Each drifted name
# differs from BOTH the fixture name and the pinned name, so nothing 0006/0007
# does can match by name -- only a match by columns converges it.
_DRIFTED_INDEXES = [
    (
        "cloudflare_images",
        ["status", "created_at"],
...
def test_upgrade_from_version_drifted_index_names(
    ...
    with django_db_blocker.unblock():
        try:
            ...
            for table, cols, fixture, drifted, _pinned in _DRIFTED_INDEXES:
                assert _meta_index_names_on(conn, table, cols) == [fixture], (
                    table,
                    cols,
                    _meta_index_names_on(conn, table, cols),
                )
                _rename_sqlite_index(conn, table, fixture, drifted, cols)
            ...
            for table, cols, _fixture, _drifted, pinned in _DRIFTED_INDEXES:
