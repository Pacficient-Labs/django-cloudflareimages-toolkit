...
Several real risk scenarios are exercised end-to-end, against real sqlite
databases and the real migration executor (not hand-rolled DDL assertions):

* :func:`test_upgrade_from_version_drifted_index_names` simulates a database
  whose physical index names were auto-generated by a *different* Django
  version than the one whose names ``0006`` hard-coded, and proves the upgrade
  still converges every index to its pinned name by matching on columns. This
  is the ``UndefinedTable``/``ProgrammingError`` abort that a plain
  ``RenameIndex(old_name=...)`` caused on such databases.
...
def _meta_index_names_on(connection, table, columns):
    """Names of the ``Meta.indexes`` indexes on exactly ``columns``.

    Filters to the ``_idx`` suffix Django gives ``Meta`` index names, so a
    column that also has ``db_index=True`` (whose implicit index has no such
    suffix) doesn't count -- the convergence assertions care only about the
    named Meta index, of which there must be exactly one.
    """
    return [n for n in _indexes_on(connection, table, columns) if n.endswith("_idx")]


def _rename_index(connection, model, fields, old_name, new_name):
    """Rename the index ``old_name`` on ``model``'s ``fields`` to ``new_name``.

    Uses Django's own schema editor (the same ``rename_index`` primitive
    0006/0007 use), which on SQLite drops the old index and recreates it under
    the new name. Simulates a database whose physical index names were
    auto-generated by a different Django version than the one whose names 0006
    hard-codes.
    """
    with connection.schema_editor() as editor:
        editor.rename_index(
            model,
            models.Index(fields=fields, name=old_name),
            models.Index(fields=fields, name=new_name),
        )


# Every affected index, as (model, index fields, legacy fixture name, a
# *drifted* name simulating a different Django version's output, pinned name).
# Each drifted name differs from BOTH the fixture name and the pinned name, so
# nothing 0006/0007 does can match by name -- only a match by columns converges
# it.
_DRIFTED_INDEXES = [
    (
        "CloudflareImage",
        ["status", "created_at"],
        "cloudflare_i_status_0b7e8c_idx",
        "cloudflare__status_8c1933_idx",
        "cfimg_status_created_idx",
    ),
    (
        "CloudflareImage",
        ["expires_at"],
        "cloudflare_i_expires_a8f9d2_idx",
        "cloudflare__expires_0ac741_idx",
        "cfimg_expires_idx",
    ),
    (
        "CloudflareImage",
        ["user", "status"],
        "cloudflare_i_user_id_b8c8a5_idx",
        "cloudflare__user_id_a3ad50_idx",
        "cfimg_user_status_idx",
    ),
    (
        "ImageUploadLog",
        ["image", "timestamp"],
        "cloudflare_i_image_i_c4e5f6_idx",
        "cloudflare__image_i_4074ed_idx",
        "cfimg_log_image_ts_idx",
    ),
    (
        "ImageUploadLog",
        ["event_type", "timestamp"],
        "cloudflare_i_event_t_d7g8h9_idx",
        "cloudflare__event_t_fcacbf_idx",
        "cfimg_log_event_ts_idx",
    ),
    (
        "ImageUsage",
        ["content_type", "object_id"],
        "cloudflare__content_9a5e2d_idx",
        "cloudflare__content_111111_idx",
        "cfimg_usage_ct_obj_idx",
    ),
    (
        "ImageUsage",
        ["cloudflare_id"],
        "cloudflare__cloudfl_e0652c_idx",
        "cloudflare__cloudfl_222222_idx",
        "cfimg_usage_cfid_idx",
    ),
]
...
def test_upgrade_from_version_drifted_index_names(
    tmp_path, settings, django_db_blocker
):
    """0006/0007 converge even when the physical index names differ from the
    ones 0006 hard-codes -- the real cross-Django-version bug.

    The historical 0001/0003 shipped their ``Meta.indexes`` WITHOUT explicit
    names, so Django auto-generated the physical names at apply time and that
    algorithm's output changed across Django versions (the prefix flipped
    ``cloudflare_i_`` -> ``cloudflare__`` and the hash changed). A database
    created under a different Django version than the one whose names 0006 baked
    into ``old_name`` therefore has index names a plain
    ``RenameIndex(old_name=...)`` cannot find -> ``migrate`` aborts with
    ``UndefinedTable``/``ProgrammingError``.

    We reproduce that faithfully: apply legacy 0001-0005, then rename every
    affected index to a name matching neither the fixture nor the pinned name,
    then upgrade onto the real (edited) 0006 + 0007. The fix must find each
    index by its columns and converge it to the single pinned name. Against the
    old hard-coded-``old_name`` 0006 this test fails at the ``migrate`` call.
    """
    from django.core.management import call_command

    alias = _fresh_sqlite_alias("drift", tmp_path, "drift.sqlite3")
    with django_db_blocker.unblock():
        try:
            call_command("migrate", "contenttypes", database=alias, verbosity=0)
            call_command("migrate", "auth", database=alias, verbosity=0)
            ...
            # Stop at 0005 so legacy 0006 (which renames by the matching
            # hard-coded names) never runs -- we want the pre-0006 physical
            # state, then we drift it ourselves.
            call_command(
                "migrate",
                APP_LABEL,
                "0005_backfill_last_referenced_at",
            ...
            # Simulate the version drift: rename every affected Meta index to a
            # name that matches neither the fixture nor the pinned name.
            for model_name, fields, fixture, drifted, _pinned in _DRIFTED_INDEXES:
                model = global_apps.get_model(APP_LABEL, model_name)
                table = model._meta.db_table
                cols = [model._meta.get_field(f).column for f in fields]
                assert _meta_index_names_on(conn, table, cols) == [fixture], (
                    table,
                    cols,
                    _meta_index_names_on(conn, table, cols),
                )
                _rename_index(conn, model, fields, fixture, drifted)
                assert _meta_index_names_on(conn, table, cols) == [drifted]

            # Upgrade the package: run the real (edited) 0006 + 0007. This is
            # the call that aborted before the fix.
            settings.MIGRATION_MODULES = {}
            call_command("migrate", APP_LABEL, database=alias, verbosity=0)

            # Convergence: exactly one Meta index per column set, pinned name.
            for model_name, fields, _fixture, _drifted, pinned in _DRIFTED_INDEXES:
                model = global_apps.get_model(APP_LABEL, model_name)
                table = model._meta.db_table
                cols = [model._meta.get_field(f).column for f in fields]
                assert _meta_index_names_on(conn, table, cols) == [pinned], (
                    table,
                    cols,
                    _meta_index_names_on(conn, table, cols),
                )

            # Idempotent re-run: nothing pending, nothing errors.
            call_command("migrate", APP_LABEL, database=alias, verbosity=0)
        finally:
            settings.MIGRATION_MODULES = {}
            _teardown_alias(alias)
