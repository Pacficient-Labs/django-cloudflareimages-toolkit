...
class CloudflareImageTransform:
    ...
    def fit(self, fit_mode: str) -> 'CloudflareImageTransform':
        ...
        if fit_mode not in valid_modes:
            raise ValueError(
                f"Fit mode must be one of: {', '.join(valid_modes)}")
    ...
    def trim(self, color: Optional[str] = None, tolerance: Optional[float] = None) -> 'CloudflareImageTransform':
        ...
        self.transforms['trim'] = ';'.join(
            trim_params) if trim_params else 'auto'
        return self
...
class CloudflareImageVariants:
    ...
    def thumbnail(base_url: str, size: int = 150) -> str:
        ...
        return (CloudflareImageTransform(base_url)
                .width(size)
                .height(size)
                .fit('cover')
                .quality(85)
...
class CloudflareImageUtils:
    ...
    def get_srcset(base_url: str, widths: List[int], quality: int = 85) -> str:
        ...
        srcset_parts = []
        for width in widths:
            transformed_url = (CloudflareImageTransform(base_url)
                               .width(width)
                               .fit('scale-down')
                               .quality(quality)
                               .format('auto')
