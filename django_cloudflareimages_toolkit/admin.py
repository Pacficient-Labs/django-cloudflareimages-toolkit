...
class CloudflareImageAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        return super().get_queryset(request).select_related('user').prefetch_related('logs')
    ...
    def cloudflare_id_display(self, obj):
        ...
        if obj.cloudflare_id:
            return format_html(
                ...
                '...' if len(obj.cloudflare_id) > 20 else obj.cloudflare_id
            )
        return '-'
    ...
    def transformation_examples(self, obj):
        ...
        if obj.is_uploaded and obj.public_url:
            ...
            examples = [
                ('Thumbnail 100px', CloudflareImageVariants.thumbnail(
                    obj.public_url, 100)),
                ('Avatar 80px', CloudflareImageVariants.avatar(obj.public_url, 80)),
                ('Product 200px', CloudflareImageVariants.product_image(
                    obj.public_url, 200)),
            ]
    ...
    class Media:
        ...
        css = {
            'all': ('admin/css/cloudflare_images_admin.css',)
        }
...
class ImageUploadLogAdmin(admin.ModelAdmin):
    ...
    def image_display(self, obj):
        ...
        if obj.image:
            url = reverse(
                'admin:django_cloudflareimages_toolkit_cloudflareimage_change', args=[obj.image.pk])
            return format_html(
                '<a href="{}">{}</a>',
                url,
                obj.image.cloudflare_id[:20] + '...' if len(
                    obj.image.cloudflare_id) > 20 else obj.image.cloudflare_id
            )
        return '-'
    ...
    def formatted_data(self, obj):
        ...
        if obj.data:
            try:
                formatted = json.dumps(obj.data, indent=2)
                return format_html('<pre style="font-size: 11px; max-height: 200px; overflow-y: auto;">{}</pre>', formatted)
            except (TypeError, ValueError):
                return str(obj.data)
        return '-'
    formatted_data.short_description = 'Data'
...
class CloudflareImagesAdminSite(admin.AdminSite):
    ...
    def index(self, request, extra_context=None):
        ...
        uploaded_images = CloudflareImage.objects.filter(
            status=ImageUploadStatus.UPLOADED).count()
        pending_images = CloudflareImage.objects.filter(
            status=ImageUploadStatus.PENDING).count()
        expired_images = CloudflareImage.objects.filter(
            expires_at__lt=timezone.now()).count()
        ...
        recent_logs = ImageUploadLog.objects.select_related(
            'image').order_by('-timestamp')[:10]
        ...
        extra_context.update({
            'cloudflare_stats': {
                ...
                'upload_success_rate': (uploaded_images / total_images * 100) if total_images > 0 else 0,
            },
